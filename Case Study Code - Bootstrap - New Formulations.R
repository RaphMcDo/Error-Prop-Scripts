...
for(boot in 1:nboot){
  ...
  for (i in 1:nrow(pred_midpoints)){
    ...
    if (pred_midpoints$model[i]==mw_models[3]) pred_midpoints$pred_mid[i]<-exp((summary(rep3)[1,1])+(summary(rep3)[3,1])*log(pred_midpoints$depth[i])+(summary(rep3)[2,1])*log(pred_midpoints$midpoints[i])-exp(summary(rep3)[4,1])^2/2)
    if (pred_midpoints$model[i]==mw_models[4]) pred_midpoints$pred_mid[i]<-exp((summary(rep4)[3,1])+(summary(rep4)[5,1])*log(pred_midpoints$depth[i])+(Report4$beta_s[pred_midpoints$loc])[i]+(summary(rep4)[4,1])*log(pred_midpoints$midpoints[i])-exp(summary(rep4)[6,1])^2/2)
    if (pred_midpoints$model[i]==mw_models[5]) pred_midpoints$pred_mid[i]<-exp((summary(rep6)[3,1])+(summary(rep6)[5,1])*log(pred_midpoints$depth[i])+((Report6$beta_s[pred_midpoints$loc])[i]+(summary(rep6)[4,1]))*log(pred_midpoints$midpoints[i])-exp(summary(rep6)[6,1])^2/2)
    ...
    if (pred_midpoints$model[i]==mw_models[8]) pred_midpoints$pred_mid[i]<-exp((summary(rep9)[5,1])+(summary(rep9)[7,1])*log(pred_midpoints$depth[i])+Report9$beta_s[pred_midpoints$loc[i]]+((Report9$beta_b_s[pred_midpoints$loc])[i]+(summary(rep9)[6,1]))*log(pred_midpoints$midpoints[i])-exp(summary(rep9)[8,1])^2/2)
  }
  ...
  for (model in 1:length(mw_models)){
    ...
    if (model == 3) new_long_heights$insh<-exp((summary(rep3)[1,1])+(summary(rep3)[3,1])*log(new_long_heights$depth)+(summary(rep3)[2,1])*log(new_long_heights$heights/100))
    if (model == 4) new_long_heights$spat<-exp((summary(rep4)[3,1])+(summary(rep4)[5,1])*log(new_long_heights$depth)+(Report4$beta_s[new_long_heights$tow_id])+(summary(rep4)[4,1])*log(new_long_heights$heights/100))
    if (model == 5) new_long_heights$spat_off<-exp((summary(rep6)[3,1])+(summary(rep6)[5,1])*log(new_long_heights$depth)+((Report6$beta_s[new_long_heights$tow_id])+(summary(rep6)[4,1]))*log(new_long_heights$heights/100))
    if (model == 6) new_long_heights$off_est<-(surv.dat7$CFh[rematch_gam])*(new_long_heights$heights/100)^(summary(rep7)[2,1])
    if (model == 7) new_long_heights$off_ln_est<-(surv.dat8$CFh[rematch_gam])*(new_long_heights$heights/100)^(summary(rep8)[2,1])
    if (model == 8) new_long_heights$spat_both<-exp((summary(rep9)[5,1])+(summary(rep9)[7,1])*log(new_long_heights$depth)+Report9$beta_s[new_long_heights$tow_id]+((Report9$beta_b_s[new_long_heights$tow_id])+(summary(rep9)[6,1]))*log(new_long_heights$heights/100))
  }
  ...
  for (model in 1:length(mw_models)){
    ...
    if (model == 3) all_small_heights$insh<-(exp((summary(rep3)[1,1])+(summary(rep3)[3,1])*log(all_small_heights$depth)+(summary(rep3)[2,1])*log(all_small_heights$pred_heights)))*all_small_heights$n
    if (model == 4) all_small_heights$spat<-(exp((summary(rep4)[3,1])+(summary(rep4)[5,1])*log(all_small_heights$depth)+(Report4$beta_s[all_small_heights$tow_id])+(summary(rep4)[4,1])*log(all_small_heights$pred_heights)))*all_small_heights$n
    if (model == 5) all_small_heights$spat_off<-(exp((summary(rep6)[3,1])+(summary(rep6)[5,1])*log(all_small_heights$depth)+((Report6$beta_s[all_small_heights$tow_id])+(summary(rep6)[4,1]))*log(all_small_heights$pred_heights)))*all_small_heights$n
    if (model == 6) all_small_heights$off_est<-((surv.dat7$CFh[match_gam_gb2])*(all_small_heights$pred_heights)^(summary(rep7)[2,1]))*all_small_heights$n
    if (model == 7) all_small_heights$off_ln_est<-((surv.dat8$CFh[match_gam_gb2])*(all_small_heights$pred_heights)^(summary(rep8)[2,1]))*all_small_heights$n
    if (model == 8) all_small_heights$spat_both<-exp((summary(rep9)[5,1])+(summary(rep9)[7,1])*log(all_small_heights$depth)+Report9$beta_s[all_small_heights$tow_id]+((Report9$beta_b_s[all_small_heights$tow_id])+(summary(rep9)[6,1]))*log(all_small_heights$pred_heights))*all_small_heights$n
  }
...
final_est_plot<-ggplot(data=subset(final_est_ci,model %in% mw_models2[c(1:5,8)]))+
  ...
  coord_cartesian(ylim=c(6000,8500))+
  theme_bw()+
